...
class DBKeeper:
    ...
    def __init__(self, folder_path: str):
        """
        Dividend Database Keeper manage the dividend data.
        Keeper will save the .db in the folder_path.
        """
        self.db_path = folder_path + "/dividend.db"
        self.__initialize()

    def update(self, symbol: str, data: dict, skipUpdated: bool = True):
        """
        Update the dividend data to database.
        ...
        if not symbol in self.mastertb:
            ...
            if not symbol in self.mastertb:
                self.__create_dividend_table(symbol)
    ...
    def query_dividend(self, symbol: str) -> dict:
        if not symbol in self.mastertb:
            # duoble check
            self.mastertb = self.master.query()
            if not symbol in self.mastertb:
    ...
    def __create_dividend_table(self, symbol: str):
        self.tb = self.db.createTB(symbol, "date", "INT")
        self.tb.addCol("dividend", "FLOAT")
        self.master.update({
            symbol: {
                "last_update": 0,
                "first_date": 0,
